//
// This program is based on the Cpp_Explore_Nodes example from the ArenaSDK
//
// The original copyright from that program is included below
//
/***************************************************************************************
 *** ***
...
//    information including display name, access mode visibility, interface
//    type, and value.
...
std::string Indent(size_t depth) {
  ...
  for (size_t i = 0; i < depth; i++) indentation += "  ";
  return indentation;
}
...
void ExploreNode(GenApi::INode* pNode, size_t depth = 0) {
  GenICam::gcstring name = pNode->GetName();
  ...
  GenICam::gcstring accessModeStr =
      GenApi::EAccessModeClass::ToString(accessMode);

  // Retrieve visibility
  GenApi::EVisibility visibility = pNode->GetVisibility();
  GenICam::gcstring visibilityStr =
      GenApi::EVisibilityClass::ToString(visibility);

  // Retrieve interface type
  GenApi::EInterfaceType interfaceType = pNode->GetPrincipalInterfaceType();
  GenICam::gcstring interfaceTypeStr =
      Arena::EInterfaceTypeClass::ToString(interfaceType);
  ...
  std::cout << Indent(depth) << name
            << std::setw(40 - name.size() - (depth * 2)) << " " << displayName
            << std::setw(50 - displayName.size() - (depth * 2)) << " ";

  if (EXPLORE_ACCESS) std::cout << accessModeStr << std::setw(5) << " ";
  ...
  if (EXPLORE_VALUE) std::cout << (value.size() < 50 ? value : "...");
